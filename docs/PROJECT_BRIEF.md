...
6. The earlier international recruitment-agency research becomes the **seed database**,
   not discarded work.
7. **No on-site or hybrid roles.** Grandeur delivers remotely, so a role requiring
   presence in an office cannot be served however strong the signal. Owner rule, set
   2026-08-26. Opportunity Research tags each signal's work arrangement from the posting
   itself, and only an **explicit** Hybrid or On-site requirement disqualifies — a
   posting that states no arrangement stays in play (owner's revision, same day: silence
   is more often remote-tolerant than not).
